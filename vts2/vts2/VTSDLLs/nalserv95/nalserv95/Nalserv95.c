/*---------------------------------------------------------------------------
	©1998-99, PolarSoft® Inc. and National Institute for Standards and Technology

module:		NALSERV95.C
desc:		BACnet Network & Application Layer Server for Windows 95 DLL v2.18
authors:	David M. Fisher, Jack R. Neyer
last edit:	01-Mar-00 [218] JN  new packet32.h, mac32.h for maximum of 2 adapters
								fix malloc and free
			16-Dec-99 [217] JN  add MSTP Support
			15-Dec-99 [216] JN	add support for passive server
			...
			25-Aug-99 [213] JN  fix segmented server receive
			18-May-99 [212] DMF add PTP support
			08-Dec-98 [211] JN  check port validity, systray, share pragma
			30-Sep-98 [210] JN  support for multiple adapters
...
//#define		maxlogicalboards	4					//									***009***210
...
///////////////////////////////////////////////////////////////////////
// Global variables
const char	szAppName[]="NALSERV95";

//NOTE:		Remember to change Version Resource Also!!
...
//NALSlboards	logicalboards[maxlogicalboards];		//								***210
int			boundadapters=0;						//									***210

/*CONTROL_BLOCK	Adapter[maxlogicalboards];			//info about our adapter(s)			***WIN95 Begin
PVOID		PacketRX[SIMULTANEOUS_READS];			//packet objects for reading		***110
//PVOID		PacketRX;								//a packet object for reading		***110
PVOID		PacketTX;								//a packet object for writing
*/
int			NEXTREAD=0;								//									***110
LPADAPTER	lpAdapter[MAXADAPTERS]={NULL,NULL};	//							***210 Begin***218
//LPPACKET	PacketRX[SIMULTANEOUS_READS]={NULL};
//LPPACKET	PacketTX=NULL;							//							***131	***210 End
TCHAR		MacDriverName[128]={"????"};			//all the space-sep MAC names LPENCI (eg.)***210
...
void	BuildNALSpacket(NALSpacket *,octet,dword,octet *,octet,BOOL,int); //***WIN95	***212
...
uint	getmaxlengthNIC (word);								//							***WIN95
...
//	create the windows we need and allocate a timer
    hStatus = CreateDialog(hDLLInst,"NALSERV95",0,NULL);
...
//	ShowWindow(hStatus,(MinimizeStatus?SW_SHOWMINIMIZED:SW_SHOWNORMAL));				***017 Delete
	...
	for (i=0;i<boundadapters;i++)				//										***210 Begin
   	{	if ((lpAdapter[i]!=NULL)&&(lpAdapter[i]->hFile!=INVALID_HANDLE_VALUE))	//This should never happen
...
//																										***012	Begin
/*	Routing is handled in NALSERV APDU transmissions according to the following guidelines:
...
*/			
	...
	*pp=(np->nalsPri&3)+(np->nalsExpectReply?4:0); 	//APDU, assume no DNET, no SNET
	...
	{
		mp->maAlen=(octet)lpAdapter[mp->maLport]->AddressLength; //get address length	***210 Begin
		if (lpAdapter[mp->maLport]!=NULL)
		{	memcpy (&mp->maSadr[0],lpAdapter[mp->maLport]->CurrentAddress,mp->maAlen);	//for debugging
			if (lpAdapter[mp->maLport]->ulAdapterType==ETH_802_3_ADAPTER)		//		***200 Begin
				memset(&mp->maDadr,0xFF,sizeof(mp->maDadr));
			else
				memset(&mp->maDadr,0,sizeof(mp->maDadr));	//							***200 End
		}													//							***210 End	***217 End
	}
...
{	int		i;
	...
	{	if(lpAdapter[i]==NULL || lpAdapter[i]->hFile==INVALID_HANDLE_VALUE)
			 return NALSE_InhibitTX;

		if (mp->maBroadcast==1)
		{	if (lpAdapter[i]->ulAdapterType==ETH_802_3_ADAPTER)	//ethernet
				memset(&mp->maDadr,0xFF,sizeof(mp->maDadr));	//let's make sure
			else
				memset(&mp->maDadr,0,sizeof(mp->maDadr));		//let's make sure
		}
		memcpy (&mp->maSadr[0],lpAdapter[i]->CurrentAddress,mp->maAlen);	//for debugging
	}												//									***212 End
...
{	static BOOL	Inited,Dirty;
    ...
    { case WM_INITDIALOG:
		...
		CheckDlgButton(hDlg,IDC_PROMISCUOUS,ispromiscuous);		
...
//here, its either a local APDU, or a broadcast APDU so we will want AL processing
...
//	out:	true/false
//
BOOL IsForUs (octet lport,octet *daddr)						//				***WIN95	***212
{	word	i;
	...
	for (i=0;i<boundadapters;i++)							//							***210 Begin
	{	if ((lpAdapter[i]!=NULL) && (lpAdapter[i]->hFile!=INVALID_HANDLE_VALUE))
		{	if (memcmp(daddr,lpAdapter[i]->CurrentAddress,
							 lpAdapter[i]->AddressLength)==0) //						***210 End
				return true;
		}
	}
	return false;
...
//	out:	true/false
//
BOOL IsEcho (octet lport,octet *saddr)				//						***WIN95	***212
...
void BuildNALSpacket(NALSpacket *np,octet lport,dword dnet,octet far *radr,octet rlen, //	***214
					BOOL expectreply,int priority)			//***WIN95
...
{	return mymalloc(size);									//use the one in packet32 ***218
//return GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,size);//allocate space		***218
}
...
{	myfree(mp);													//use the one in packet32 ***218														
	//GlobalFreePtr(mp);										//						***218
}																		//				***100 End
//																		//				***100 Begin
...
// These routines are different than their NT versions primarily in the
// interface to the Packet32 routinse. The main difference is that Packet32
// allocates and deallocates the actual RX/TX buffers in the Packet32Init
// and Packet32Free routines.
...
word MACPInit()
{	UINT	NameLength=128;										//	***WIN95			***210 Begin
	char	b[128],p[128],*bp,*pp;								//						***210
	TCHAR	AdapterName[]=TEXT("BACMAC95_0");					//						***210
	ULONG	Filter;
	int		i,j;												//	***110				***210
	LPPACKET lpPacket;
	BOOLEAN	status;
...
//	Open the first adapter										//						***210 Begin
	lpAdapter[0]=Packet32OpenAdapter(AdapterName);
	if (!lpAdapter[0]||lpAdapter[0]->hFile==INVALID_HANDLE_VALUE) return MAE_OpenAdapterFailed;	//we have to have at least one adapter

//	Get MacDriver name
	lpPacket=Packet32AllocatePacket(lpAdapter[0],false);		//						***140
	if (lpPacket)
	{	if (Packet32InitPacket(lpPacket,NameLength))
		{	Packet32GetAdapterNames(lpAdapter[0],lpPacket);
			memcpy(&MacDriverName,lpPacket->Buffer,NameLength);
		}
		Packet32FreePacket(lpPacket);
	}
//	Determine the number of bound adapters from asciiz MacDriverName=name0 name1 ... namex 0
	boundadapters=1;										//we know we have at least one
	bp=MacDriverName;
	for (i=0;i<MAXADAPTERS;i++)
	{	if (i!=0)											//we already know about the first
		{	AdapterName[9]='0'+i;
			lpAdapter[i]=Packet32OpenAdapter(AdapterName);
			boundadapters++;								//bump board count
		}
		pp=strchr(bp,(int)' ');
		*pp=0;												//temp
		strcpy(b,bp);										//get the name
		*pp++=' ';						
		bp=pp;												//bump to next name
		sprintf(p,"Logical Bound Adapter[%u] = %s",i,b);
		MakeLog (p,false);
//	Set packet filters
		Filter=((ispromiscuous)&&(lpAdapter[i]->ulAdapterType==ETH_802_3_ADAPTER))?NDIS_PACKET_TYPE_PROMISCUOUS:NDIS_PACKET_TYPE_DIRECTED|NDIS_PACKET_TYPE_BROADCAST;	//	***NALS***200***210
		status=Packet32SetFilter(lpAdapter[i],Filter);
//	Now Start Some Reads												//				***110 Begin
		for (j=0;j<MAXREADREQS;j++)
		{	lpAdapter[i]->PacketRX[j]=Packet32AllocatePacket(lpAdapter[i],true);		//					***140
			if (lpAdapter[i]->PacketRX[j])
			{	if (lpAdapter[i]->ulAdapterType==ETH_802_3_ADAPTER)					//	***200 Begin
					n=ETH_802_3_HEADER_LENGTH;			
				else
					n=ARCNET_HEADER_LENGTH;
				if (Packet32InitPacket(lpAdapter[i]->PacketRX[j],lpAdapter[i]->ulMaxFrameSize+n))	//	***200 End
				{	RXEvents[(i*MAXREADREQS)+j]=((LPPACKET)lpAdapter[i]->PacketRX[j])->OverLapped.hEvent;			//save event handle		***140
					Packet32ReceivePacket(lpAdapter[i],lpAdapter[i]->PacketRX[j],false);				//don't wait
				}
				else 
				{	Packet32FreePacket(lpAdapter[i]->PacketRX[j]);
					lpAdapter[i]->PacketRX[j]=NULL;
				}
			}
			else
				return MAE_NoFreeBuffers;
		}													//							***110 End
		lpAdapter[i]->commEnabled=true;
		lpAdapter[i]->commEnableTimer=0;
		if (*pp==0) break;									//no more bound adapters
	}														//							***210 End
//																						***140 End
...
//	Reset the Adapter
//	Packet32ResetAdapter(lpAdapter);					//shut down events??			***128
	for (i=0;i<boundadapters;i++)						//								***210 Begin
	{
//	Free up the packets we got
		for (j=0;j<MAXREADREQS;j++)						//								***110 Begin
		{	ResetEvent(RXEvents[(i*MAXREADREQS)+j]);		//reset the event signal manually
			Packet32FreePacket(lpAdapter[i]->PacketRX[j]);
		}												//								***110 End
//	Close it up
		Packet32CloseAdapter(lpAdapter[i]);
	}													//								***210 End
		...
		{	NEXTREAD=dw-WAIT_OBJECT_0;
			...
			{	done=true;										//default case (will be true only when no more events to process)
				...
				{	lport=NEXTREAD/MAXREADREQS;					//calc the logical port (adapter)
					...
					if ((lpAdapter[lport]->PacketRX[levent]==NULL)||
						((Result=Packet32WaitPacket(lpAdapter[lport],lpAdapter[lport]->PacketRX[levent],&BytesReceived,false))==ERROR_IO_INCOMPLETE))	//***131
...
//																						***200 Begin
						if (lpAdapter[lport]->ulAdapterType==ETH_802_3_ADAPTER)
						{	pkt=(PKT8022 *)lpAdapter[lport]->PacketRX[levent]->Buffer;
							if (IsEcho((octet)lport,pkt->pktSADR)) break;		//it's an echo
						}
						else
						{	apkt=(PKTARC *)lpAdapter[lport]->PacketRX[levent]->Buffer;
							if (IsEcho((octet)lport,&apkt->ApktSID)) break;		//it's an echo
						}
...
//						if ((mp=GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,sizeof(MApacket)))==NULL)	***218
						if ((mp=GMALLOC(sizeof(MApacket)))==NULL)				//						***218
						{	RxNoMAs++;
							break;							//toss it!!!
						}
						if (lpAdapter[lport]->ulAdapterType==ETH_802_3_ADAPTER)	//Ethernet
						{	mp->maLsap=pkt->pktLLC[0];				//get lsap
							...
							mp->maAlen=6;							//address length for media type
...
/*						pkt=(PKT8022 *)PacketRX[NEXTREAD]->Buffer;
...
//		Free the last event
					Packet32FreePacket(lpAdapter[lport]->PacketRX[levent]);
//		Start another receive event
					lpAdapter[lport]->PacketRX[levent]=Packet32AllocatePacket(lpAdapter[lport],true);
					if (lpAdapter[lport]->PacketRX[levent])
					{	if (Packet32InitPacket(lpAdapter[lport]->PacketRX[levent],lpAdapter[lport]->ulMaxFrameSize+n))	//***200
						{	Packet32ReceivePacket(lpAdapter[lport],lpAdapter[lport]->PacketRX[levent],false);	 //don't wait
							RXEvents[NEXTREAD]=((LPPACKET)lpAdapter[lport]->PacketRX[levent])->OverLapped.hEvent;
						}
						else
						{	Packet32FreePacket(lpAdapter[lport]->PacketRX[levent]);
							lpAdapter[lport]->PacketRX[levent]=NULL;
						}
					}
					if ((++NEXTREAD)>=maxevents) NEXTREAD=0;
					done=false;
...
//out:	pointer to a buffer containing the message
...
//
void MACPtransmit(MApacket *mp)
{	byte	*bp;									//for easy access
	byte	b[2048];
	word	n,nmax;
	int		i,lport;								//									***121
	dword	BytesReceived;							//									***120 Begin
	
	lport=(int)mp->maLport;							//									***121 Begin
	...
	if (lpAdapter[lport]->PacketTX!=NULL)			//we have a transmit in progress
	{	Packet32WaitPacket(lpAdapter[lport],lpAdapter[lport]->PacketTX,&BytesReceived,true);
		Packet32FreePacket(lpAdapter[lport]->PacketTX);
	}
	lpAdapter[lport]->PacketTX=Packet32AllocatePacket(lpAdapter[lport],false);		//						***140
	if (!lpAdapter[lport]->PacketTX) return;							//we couldn't allocate a packet	***120	End
	if (lpAdapter[lport]->ulAdapterType==ETH_802_3_ADAPTER)		//				***200	***210
	{	nmax=(word)(lpAdapter[lport]->ulMaxFrameSize+ETH_802_3_HEADER_LENGTH);
//	build the packet to send
	...
	{	nmax=(word)(lpAdapter[lport]->ulMaxFrameSize+ARCNET_HEADER_LENGTH);
//	build the packet to send
		bp=b;
	...
	n=bp-b;												//n is the number of bytes to send
	if (n<=nmax)
	{	if (Packet32InitPacket(lpAdapter[lport]->PacketTX,nmax))			//want to allocate max amount	***200
														//because ARCNET NDIS driver 
														//reorders and adds to buffer
														//--Actually this doesn't seem to matter
		{	memcpy(lpAdapter[lport]->PacketTX->Buffer,b,n);
			lpAdapter[lport]->PacketTX->Length=n;							//only want to transmit this much	***200
			Packet32SendPacket(lpAdapter[lport],lpAdapter[lport]->PacketTX,false);	//no WAIT for COMPLETION	***110***120
		}
	}
	else											//									***120 Begin
	{	Packet32FreePacket(lpAdapter[lport]->PacketTX);
		lpAdapter[lport]->PacketTX=NULL;
	}												//									***120 End
//																						***210 End
}
//																						***WIN95 End
...
{	if(lport==ptpport) return 0;					//ptp is special					***212
	...
	return Packet32GetAddress (lpAdapter[lport],(PBYTE)paddr);	//			***WIN95	***210
}	
...
{	if(lport==ptpport||lport==mstpport) return 501;			//ptp/mstp is special		***212***217
	return	(word)(lpAdapter[lport]->ulMaxFrameSize-3);		//				***200		***210
}
...
{	if(lbd==ptpport)						//treat PTP specially here					***212
	...
	if(lbd==mstpport)						//treat MSTP specially here					***217
		return 1;							//											***217
	return (word)lpAdapter[lbd]->AddressLength;					//						***210
}	

///////////////////////////////////////////////////////////////////////
//	Get List of Logical Boards
word MACPgetportlist(char *blist)
{	char	b[4096],*bp,*pp;				//											***210
	char	lpt[]={"000 "};					//											***210
	word	n;

	b[0]=0;
/*	strcat(b,"000 ");						//for now only one board number				***210 Begin
	strcat(b,MacDriverName);				//append name
	strcat(b,"\r\n");						//crlf
*/
	bp=MacDriverName;
	for (n=0;n<boundadapters;n++)
	{	strcat(b,lpt);
		pp=strchr(bp,(int)' ');				//find next name
		*pp=0;								//temp
		strcat(b,bp);						//get the name
		strcat(b,"\r\n");					//crlf
		*pp++=' ';						
		bp=pp;								//bump to next name
		lpt[2]++;
