...
//from TESTAPP.H
//#define SIMULTANEOUS_READS	12		//									***110***131***210
#define		MAXADAPTERS					2					//				***210***218
#define		MAXREADREQS					24					//				***210***218
// Caution: MAXADAPTERS*MAXREADREQS cannot exceed MAXIMUM_WAIT_OBJECTS=64!	***273
typedef struct _CONTROL_BLOCK {
	PVOID	hFile;					//a pointer to an adapter object (not a file handle)
	HANDLE	hEvent;					//not used
	TCHAR	AdapterName[64];		//the UNICODE NT adapter name \Device\bacmac1
	TCHAR	MacDriverName[64];		//the UNICODE NT MAC name \Device\DC21X41 (eg.) ***001
//																		***110 Begin
	HGLOBAL	hMem[MAXREADREQS];		//handle to an allocated block of memory (for RX)***210
	LPBYTE	lpMem[MAXREADREQS];		//pointer to the memory block					***210
//																		***110 End
	...
	PVOID	PacketRX[MAXREADREQS];	//packet objects for reading		***210 Begin
	PVOID	PacketTX;				//a packet object for writing
	word	Alen;					//length of address
	octet	Adr[8];					//address
	dword	AdapterType;			//0=ethernet, 1=ARCNET
	DWORD	commEnableTimer;		//a timer for re-enabling communications (in seconds)
	BOOL	commEnabled;			//true if communications are enabled	***210 End

	ULONG	LastReadSize;			//not used
	UINT	BufferSize;				//the max size of a network packet (1514)
	word	MaxFrameSize;			//										***219
...
#define	MAE_CreateThreadFailed		 -4				//					***140
...
__declspec(dllexport) BOOL MACPreceive(MApacket far *);
//word MACPgetlen(word);
__declspec(dllexport) word MACPgetaddr(word,octet far *);
__declspec(dllexport) void MACPtransmit(MApacket far *);
//MACPgetportlist(char far *);
__declspec(dllexport) word MACPmaxNPDUlength(word);
__declspec(dllexport) void MACPgetstats(MACStatTable far *);
